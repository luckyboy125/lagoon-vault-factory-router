...
describe("MockLagoonVault", function () {
  ...
  describe("Synchronous Deposits", function () {
    ...
    it("Should allow sync deposit", async function () {
      ...
      await expect(
        mockLagoonVault.connect(user1).syncDeposit(depositAmount, user1.address, user2.address)
      ).to.emit(mockLagoonVault, "SyncDeposit")
        .withArgs(user1.address, user2.address, depositAmount, depositAmount);

      expect(await mockLagoonVault.balanceOf(user1.address)).to.equal(depositAmount);
    ...
    it("Should transfer assets from user", async function () {
      ...
      await mockLagoonVault.connect(user1).syncDeposit(depositAmount, user1.address, user2.address);
      
      expect(await mockUSDC.balanceOf(user1.address)).to.equal(userBalanceBefore - depositAmount);
    ...
    it("Should revert if amount is zero", async function () {
      await expect(
        mockLagoonVault.connect(user1).syncDeposit(0, user1.address, user2.address)
  ...
  describe("Asynchronous Deposits", function () {
    ...
    it("Should revert async deposit if vault is ready for sync", async function () {
      ...
      await expect(
        mockLagoonVault.connect(user1).requestDeposit(depositAmount, user1.address, user1.address)
    ...
    it("Should allow claiming async deposit when vault becomes ready", async function () {
      ...
      await expect(
        ...
        .withArgs(requestId, user1.address, depositAmount);
  ...
  describe("Total Assets", function () {
    it("Should track total assets correctly", async function () {
      ...
      await mockLagoonVault.connect(user1).syncDeposit(depositAmount1, user1.address, ethers.ZeroAddress);
      expect(await mockLagoonVault.totalAssets()).to.equal(depositAmount1);
      
      await mockLagoonVault.connect(user2).syncDeposit(depositAmount2, user2.address, ethers.ZeroAddress);
